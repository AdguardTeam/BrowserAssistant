...
# Safety: refuse to run outside Bamboo CI to prevent accidental local execution.
if [ -z "${bamboo_buildNumber:-}" ]; then
    echo "ERROR: this script is intended to run in Bamboo CI only (bamboo_buildNumber is not set)."
    exit 1
fi
...
# WARNING: this wipes the entire workspace including .git — only run in a disposable CI workspace.
